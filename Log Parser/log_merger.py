...
def _touch_event_replacer(event_id):
...
def process_file(game_filename, touch_filename):
    ...
    g_df = pd.read_csv(game_filename, header=None)
    # print(g_df.head())
    # Rename columns
    ...
    t_df = pd.read_csv(touch_filename, header=None)
    # print(t_df.head())
    # Rename columns
    ...
    t_df['touch-event'] = t_df['touch-event'].map(_touch_event_replacer)
    # Merging tables
    merge(g_df, t_df)
    return g_df
